...
public class NetworkIdle {
    ...
    public static void main(String[] args) {
        ...
        int res = solution.networkBecomesIdle(new int[][]{
                {0,1}, {1,2}
...
//                0,5,4,6,8,6,10,3,11,1,9,1,9,13,9,9,12,3,6,7,2,5,1,4,7,4,18,10,7,9,16,1,24,1,5,15,5,8,11,2,27,4,10,1,3,4,3,8,1,10,12,21,3,2,17,1,1,1,5,3,4,29,11,2,4,2,9,9,12,9,2,10,9,7,5,7,9,3,10,2,7,4,8,3,12,4,16,6,4,22,6,13,20,1,2,8,1,2,5,16,29,16,5,1,27,2,5,22,1,6,4,5,7,5,15,5,4,5,4,6,8,5,8,24,23,17,7,8,3,1,7,5,2,17,4,5,11,9,16,1,12,13,26,1,7,4,18,1,7,12,19,12,1,10,3,3,14,8,15,19,22,1,4,22,14,1,4,1,11,11,13,2,2,1,1,7,8,2,2,1,15,3,4,19,1
    ...
    static class Solution {
        public int networkBecomesIdle(int[][] edges, int[] patience) {
            int[] weight = new int[patience.length];
            for (int i = 0; i < patience.length; i++) {
                weight[i] = Integer.MAX_VALUE;
            }
            weight[0] = 0;
            for (int j = 0; j < patience.length; j++) {
                for (int[] edge : edges) {
                    int s = edge[0];
                    int d = edge[1];
                    if (weight[s] != Integer.MAX_VALUE && weight[s] + 1 < weight[d]) {
                        weight[d] = weight[s] + 1;
                    }
                }
            }
            ...
            for (int i = 1; i < patience.length; i++) {
                int rtt = weight[i] * 2;
                int extraPackets = (rtt - 1) / patience[i];
                int curr =  extraPackets * patience[i] + rtt + 1;
                if(curr > max){
                    max = curr;
                }
            }
